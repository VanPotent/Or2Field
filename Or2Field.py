...
def convert_orion_to_rayfield(orion_code: str) -> str:
    ...
    for i, line in enumerate(lines):
        # check for loadstring and any of the Orion identifiers (case‐sensitive variants)
        if 'loadstring' in line and re.search(r'\b(?:OrionLib|orionlib|orionLib|Orionlib|Orion|orion)\b', line):
            lines[i] = 'local Rayfield = loadstring(game:HttpGet(\'https://sirius.menu/rayfield\'))()'
    text = "\n".join(lines)



    # 2. OrionLib:MakeWindow call to Rayfield:CreateWindow
