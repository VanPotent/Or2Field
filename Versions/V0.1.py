...
def convert_orion_to_rayfield(orion_code: str) -> str:
    ...
    def replace_makewindow(match):
        ...
        if cc_idx != -1:
            # Find end of the CloseCallback function block
            ...
            if func_idx != -1:
                ...
                while pos < len(content):
                    ...
                    if nest == 0:
                        ...
                        continue
                if end_idx != -1:
                    # Include the 'end' and remove the whole CloseCallback segment
                    end_idx += len('end')
                    # Remove trailing comma after CloseCallback if any
                    after = content[end_idx:]
                    if after.lstrip().startswith(','):
                        end_idx += after.find(',') + 1
                    content = (content[:cc_idx].rstrip().rstrip(',')).rstrip()
        # Parse Window settings in Orion
        ...
        intro_icon_val = None
        if introicon_match:
            # Convert IntroIcon if given (use as topbar Icon if possible)
        ...
        if intro_enabled:
            # Map intro text to loading title if provided
            if intro_text_val:
                fields.append(f'LoadingTitle = {intro_text_val}')
            if intro_icon_val:
                # If IntroIcon was provided, we could use it as Icon (already handled above) or LoadingSubtitle
        ...
        return f'Rayfield:CreateWindow({{{new_content}}})'
    ...
    text = re.sub(r'(\w+):AddParagraph\(\s*(".*?"|\'.*?\')\s*,\s*(".*?"|\'.*?\')\s*\)', replace_addparagraph, text)
    ...
    # 13. Convert OrionLib:MakeNotification to Rayfield:Notify
    def replace_notification(match):
        ...
        title_val = title_match.group(1) if title_match else '""'
        content_val = content_match.group(1) if content_match else '""'
        image_field = None
        if image_match:
            raw = image_match.group(1)
            if raw[0] in ('"', "'"):
                inner = raw[1:-1]
                if inner.startswith("rbxassetid://"):
                    image_field = f'Image = {inner.split("://")[1]}'
                elif inner.isdigit():
                    image_field = f'Image = {inner}'
                else:
                    image_field = f'Image = "{inner}"'
            else:
                image_field = f'Image = {raw}'
        duration_field = None
        if time_match:
            duration_field = f'Duration = {time_match.group(1)}'
        fields = [f'Title = {title_val}', f'Content = {content_val}']
        if image_field:
